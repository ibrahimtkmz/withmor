...
// --- GALERİ RESİMLERİ (BURAYA RESİMLERİNİZİ EKLEYEBİLİRSİNİZ) ---
// Kullanım: image alanına resim yolunu veya URL'sini yazın.
const GALLERY_IMAGES = [
  { id: 1, src: "https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&q=80&w=800", alt: "Asansör Montaj 1" },
  { id: 2, src: "https://images.unsplash.com/photo-1546410531-bb4caa6b424d?auto=format&fit=crop&q=80&w=800", alt: "Kabin İçi 1" },
  { id: 3, src: "https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?auto=format&fit=crop&q=80&w=800", alt: "Çelik Yapı" },
  { id: 4, src: "https://images.unsplash.com/photo-1581092334651-ddf26d9a09d0?auto=format&fit=crop&q=80&w=800", alt: "Teknik Servis" },
  { id: 5, src: "https://images.unsplash.com/photo-1535557597501-0febd41569f1?auto=format&fit=crop&q=80&w=800", alt: "Yük Asansörü" },
  { id: 6, src: "https://images.unsplash.com/photo-1579621970795-87facc2f976d?auto=format&fit=crop&q=80&w=800", alt: "Panoramik Asansör" },
];

// --- TRANSLATION DATA ---
const TRANSLATIONS = {
  tr: {
    menu: { about: "Kurumsal", services: "Hizmetler", projects: "Projeler", references: "Referanslar", contact: "İletişim", gallery: "Galeri" },
    hero: {
    ...
    gallery: {
        ...
        subtitle: "Projelerimizden ve uygulamalarımızdan kareler."
    },
    reviews: {
  ...
  en: {
    menu: { about: "About Us", services: "Services", projects: "Projects", references: "References", contact: "Contact", gallery: "Gallery" },
    hero: {
    ...
    gallery: {
        title: "Photo Gallery",
        subtitle: "Snapshots from our projects and applications."
    },
    reviews: {
  ...
  ru: {
    menu: { about: "О нас", services: "Услуги", projects: "Проекты", references: "Референсы", contact: "Контакты", gallery: "Галерея" },
    hero: {
    ...
    gallery: {
        title: "Фотогалерея",
        subtitle: "Снимки наших проектов и приложений."
    },
    reviews: {
  ...
  ar: {
    menu: { about: "من نحن", services: "خدماتنا", projects: "المشاريع", references: "المراجع", contact: "اتصل بنا", gallery: "صور" },
    hero: {
    ...
    gallery: {
        title: "معرض الصور",
        subtitle: "لقطات من مشاريعنا وتطبيقاتنا."
    },
    reviews: {
      title: "تجربة العملاء",
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen bg-white text-slate-800 font-sans selection:bg-blue-100 overflow-x-hidden ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-6 text-sm font-medium text-slate-600 md:flex">
            ...
            <a href="#gallery" className="hover:text-blue-700 transition-colors">{t.menu.gallery}</a>
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-[100] bg-white h-[100dvh] flex flex-col animate-in slide-in-from-right duration-200 md:hidden">
             ...
             <nav className="flex flex-col p-6 gap-6 text-lg font-medium text-slate-700">
                ...
                <a href="#gallery" onClick={() => setMobileMenuOpen(false)}>{t.menu.gallery}</a>
      ...
      {/* ... Hero, About, Services, Projects sections ... */}
      {/* (Existing sections remain unchanged, inserting Gallery before References) */}
      ...
      <main className="w-full bg-white">
        ...
        {/* NEW: FOTOĞRAF GALERİSİ */}
        <section id="gallery" className="scroll-mt-24 max-w-6xl mx-auto px-6 py-20 border-t border-slate-100">
           <div className="flex flex-col items-center text-center mb-12">
              <span className="text-xs font-bold uppercase tracking-wider text-blue-700 mb-2">{t.menu.gallery}</span>
              <h2 className="text-3xl font-bold text-slate-900">{t.gallery.title}</h2>
              <p className="text-sm text-slate-500 mt-2 max-w-xl">{t.gallery.subtitle}</p>
           </div>
                ...
                >
                   ...
                   <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
                      <div className="bg-white/90 p-2 rounded-full opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                         <Icons.ZoomIn size={20} className="text-slate-800" />
                      </div>
                   </div>
                </div>
              ))}
      ...
      {selectedImage && (
         <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4" onClick={() => setSelectedImage(null)}>
            <button className="absolute top-4 right-4 text-white p-2 hover:bg-white/10 rounded-full"><Icons.X size={32}/></button>
            <img src={selectedImage.src} alt={selectedImage.alt} className="max-w-full max-h-[90vh] rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()} />
            <div className="absolute bottom-8 left-0 right-0 text-center text-white font-medium text-lg drop-shadow-md pointer-events-none">{selectedImage.alt}</div>
         </div>
      )}
    </div>
  );
