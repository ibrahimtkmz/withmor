...
export default function App() {
  ...
  // HİZMETLER İÇİN YEREL GÖRSELLER + UZUN SEO METİNLERİ
    const [services, setServices] = useState([
    {
      ...
      longDesc: `Hidrolik yük asansörü çözümleri, özellikle ağır sanayi tesislerinde, üretim hatlarında, depolama alanlarında ve lojistik merkezlerinde yüksek tonajlı yüklerin güvenle taşınması için geliştirilmiş sistemlerdir. Withmor olarak ürettiğimiz hidrolik yük asansörleri, hem dikey taşıma kapasitesini artırır hem de işletmelerin iç lojistik süreçlerini daha kontrollü ve planlı hale getirir. Yük platformları ve endüstriyel kaldırma sistemleriyle birlikte düşünüldüğünde, tüm malzeme akışını tek bir bütüncül mühendislik çatısı altında toplamanızı sağlar.

Hidrolik sistemin en önemli avantajı, yüksek taşıma kapasitesini görece düşük motor gücü ile elde edebilmesidir. Bu sayede, enerji tüketimi geleneksel sistemlere kıyasla daha dengeli olur ve özellikle gün boyu yoğun çalışan tesislerde ciddi bir verimlilik kazancı sağlar. Hidrolik güç ünitesi, silindirler, hortumlar ve emniyet valfleri gibi bileşenler, EN-81 ve ilgili güvenlik standartlarına uygun şekilde seçilir. Her bir bileşen, ağır sanayi koşullarında uzun yıllar boyunca sorunsuz çalışacak dayanıklılıkta tasarlanır.

Hidrolik yük asansörleri, fabrika içi sevkiyatlarda paletli yükler, kasalar, makine parçaları, bobinler ve hacimli ürünler için ideal çözümdür. Depo katları arasında ürün transferi yapılırken hem operatör güvenliği hem de ürün emniyeti ön plandadır. Bu yüzden kabin tasarımı, zemin kaplaması, korkuluklar, fotoseller, kapı tipleri ve kumanda elemanları her işletmenin gerçek kullanım senaryosuna göre şekillendirilir. Yük platformları ile birlikte kullanıldığında tır yükleme – boşaltma operasyonlarında da önemli hızlanma sağlar.

Bir diğer önemli nokta da hidrolik yük asansörlerinin mimari açıdan sağladığı esnekliktir. Kuyu dibi ve tepe boşluğu gereksinimleri, klasik sistemlere göre daha avantajlı seviyelere çekilebilir; bu da mevcut binalarda iyileştirme yapılırken veya yeniden düzenleme projelerinde ciddi bir avantaj sunar. Özellikle eski fabrikaların veya kısıtlı kuyu ölçülerine sahip yapıların modernizasyonunda, hidrolik sistemler maliyet–performans dengesi açısından öne çıkar.

Withmor mühendislik ekibi, her hidrolik yük asansörü projesinde önce sahayı analiz eder, yük tiplerini, günlük taşıma adetlerini, operasyon sürelerini ve mevcut bina koşullarını detaylı şekilde inceler. Ardından, taşıma kapasitesi, hız, kabin ölçüleri, kapı tipi ve kumanda senaryoları belirlenerek işletmeye özel bir çözüm tasarlanır. İmalatı ve montajı tamamlanan sistemler, test ve devreye alma süreçlerinde tam yük altında denenir; böylece gerçek kullanım koşullarına hazır hale getirilir.

Periyodik bakım ve servis süreçleri de hidrolik yük asansörlerinin güvenli çalışması için kritik öneme sahiptir. Yağ kalitesi ve seviyesi, hortum ve bağlantı elemanlarının kontrolü, emniyet valflerinin fonksiyon testleri ve genel mekanik kontroller düzenli aralıklarla yapılmalıdır. Withmor servis ekibi, Türkiye’nin farklı bölgelerinde kurulan hidrolik yük asansörleri için planlı bakım ve hızlı müdahale hizmeti sunar. Böylece üretim ve lojistik operasyonlarınız, kesintisiz bir şekilde devam ederken, yük asansörleri ve yük platformları işletmenizin güvenilir birer çalışma arkadaşı haline gelir.`
    },
    {
      id: "mrl-yuk",
      ...
      longDesc: `Makine dairesiz yük asansörü çözümleri, modern mimarinin vazgeçilmez unsurlarından biri haline gelmiştir. Özellikle otopark blokları, ticari yapılar, lojistik merkezleri ve karma kullanımlı binalarda, asansör sistemlerinin minimum hacimle maksimum performans sağlaması beklenir. Withmor makine dairesiz yük asansörleri (MRL), dişlisiz motor teknolojisini gelişmiş kumanda sistemleriyle birleştirerek hem enerji verimliliği hem de sessiz çalışma açısından işletmelere önemli avantajlar sunar.

Klasik makine daireli sistemlerde motor ve makine grubunun ayrı bir oda içinde konumlandırılması gerekirken, MRL yük asansörlerinde tahrik ünitesi kuyu içinde, özel bir konstrüksiyon üzerinde yer alır. Bu sayede binada makine dairesi için ekstra bir alan ayrılmasına gerek kalmaz; mimari proje daha esnek şekilde kurgulanabilir. Özellikle kat sayısı yüksek, otopark ve servis alanları yoğun binalarda bu esneklik, hem ilk yatırım maliyetlerine hem de uzun vadeli mekân kullanımına olumlu yansır.

Makine dairesiz yük asansörü sistemlerinde kullanılan dişlisiz senkron motorlar, daha düşük enerji tüketimi, daha az titreşim ve daha sessiz çalışma karakterine sahiptir. Bu tür motorlar, frekans kontrollü sürücüler ile birlikte kullanılarak kalkış ve duruş anlarında son derece konforlu bir hareket sağlar. Yük asansörleri için konfor, sadece insan asansörleri kadar ön planda değilmiş gibi görünse de, hem ürün emniyeti hem de operatör deneyimi açısından titreşimsiz ve sarsıntısız taşıma son derece önemlidir.

Withmor olarak MRL yük asansörlerini tasarlarken, her projede öncelikle taşıma kapasitesi, kullanım sıklığı ve bina içi trafik yoğunluğunu analiz ediyoruz. Servis alanlarında, otopark giriş–çıkış noktalarında, depo ve mağaza bağlantılarında hangi tip yüklerin hangi saat aralıklarında taşındığı detaylı şekilde incelenir. Bu veriler, motor gücü, hız, kabin ölçüleri, kapı genişliği ve kontrol senaryolarının belirlenmesinde doğrudan rol oynar. Böylece hem yoğun kullanım saatlerinde performans kaybı yaşanmayan hem de enerji tasarrufu sağlayan bir sistem kurgulanır.

Makine dairesiz yük asansörlerinin bir diğer avantajı da mimari estetikle daha uyumlu çözümler sunmasıdır. Kuyu üst kotunda ekstra bir oda ihtiyacı olmaması, çatı silueti ve bina cephesi tasarımını kolaylaştırır. Özellikle AVM’lerde ve karma yapılarda, yük asansörleri görünür alanlara yakın konumlandırılmak zorunda olduğunda, MRL teknolojisi ile hem teknik gereksinimler karşılanır hem de mimari çizgi korunur.

Servis ve bakım süreçleri de makine dairesiz yük asansörlerinde büyük önem taşır. Withmor servis ekipleri, dişlisiz motor grupları, fren mekanizmaları, süspansiyon sistemleri ve kumanda panolarını düzenli olarak kontrol ederek sistemin güvenli ve verimli çalışmasını sağlar. Kullanıcıların ve işletmelerin güvenliği için tüm bakım işlemleri, ilgili standartlar ve yönetmeliklere uygun şekilde kayıt altına alınır. Böylece MRL yük asansörleriniz, uzun yıllar boyunca kesintisiz ve güvenilir bir şekilde işletmenizin parçası olmaya devam eder.`
    },
    {
      id: "homelift",
      name: "Homelift",
      ...
      longDesc: `Homelift sistemleri, modern yaşam alanlarında konforu, güvenliği ve erişilebilirliği bir arada sunan çözümlerdir. Müstakil evler, villalar, dubleks ve triplex konutlar, butik rezidanslar ve ofis–konut karması projelerde homelift asansörleri, hem günlük yaşamı kolaylaştıran bir ulaşım aracı hem de prestij unsuru olarak öne çıkar. Withmor homelift çözümleri, minimal kuyu dibi ve tepe boşluğu ihtiyaçları sayesinde mevcut yapılara da sonradan kolayca entegre edilebilir.

Ev tipi asansör ihtiyaçlarında en önemli konu, kullanıcı profilinin çok farklı olmasıdır. Çocuklar, yaşlılar, misafirler ve hareket kısıtlılığı olan bireyler aynı sistemi kullanabilir. Bu nedenle homelift projelerinde güvenlik donanımları, kumanda elemanlarının ergonomisi ve kabin içi tasarım detayları büyük önem taşır. EN-81 ve ilgili standartlara uygun olarak tasarlanmış Withmor homelift sistemleri, acil durum butonları, çift taraflı haberleşme, fotosel perde, aşırı hız regülatörü ve emniyetli kapı kilitleme mekanizmaları ile kullanıcıyı her katta koruma altına alır.

Homelift çözümleri estetik açıdan da yaşam alanlarınızla uyumlu olacak şekilde geliştirilir. Cam şaftlı, tam cam kabinli, paslanmaz çelik, ahşap veya özel kaplamalı tasarımlar ile evinizin iç mimarisine uyum sağlayan birçok alternatif sunarız. Merdiven kovası içine konumlandırılan sistemlerden bağımsız cam kule çözümlerine kadar farklı uygulamalar sayesinde; ister yeni bir villa projesinde, ister mevcut dubleks evinizde homelift tercih ederek katlar arasında rahatça hareket edebilirsiniz.

Enerji tüketimi, ev tipi asansör seçiminde önemli bir kriterdir. Withmor homelift sistemleri, yüksek verimli motorlar ve frekans kontrollü sürücüler kullanır. Bu sayede hem kalkış ve duruş anlarında konforlu bir hareket elde edilir hem de elektrik sarfiyatı kontrollü bir seviyede tutulur. Yıllık bakım ve işletme maliyetleri, klasik asansör sistemlerine göre daha erişilebilir seviyelerdedir. Bu da homelift yatırımlarını uzun vadede ekonomik kılar.

Özellikle yaşlanan nüfus ve erişilebilirlik beklentilerinin artmasıyla birlikte, villa asansörleri ve homelift çözümleri gayrimenkul değerinin yükselmesinde de önemli rol oynamaya başlamıştır. Evinizde asansör bulunması, hem günlük yaşam kalitesini yükseltir hem de uzun vadede mekânın fonksiyonelliğini artırır. Hareket kısıtlılığı olan kullanıcılar için merdiven riskini ortadan kaldırır; ağır eşyaların, market poşetlerinin veya çocuk arabalarının katlar arasında güvenle taşınmasını sağlar.

Withmor olarak her homelift projesinde, önce yaşam alanınızı ve beklentilerinizi detaylı şekilde analiz ediyoruz. Kuyu ölçüleri, taşıyıcı sistem, mimari detaylar ve kullanım alışkanlıklarınız doğrultusunda en uygun homelift çözümünü geliştiriyor, projeyi anahtar teslim teslim ediyoruz. Uzun yıllar boyunca güvenle kullanabileceğiniz, sessiz, konforlu ve şık ev asansörü sistemleriyle yaşam kalitenizi bir üst seviyeye taşıyoruz.`
    },
    {
      id: "insan-asansoru",
      name: "İnsan Asansörü",
      ...
      longDesc: `İnsan asansörü sistemleri, konut projelerinde, iş merkezlerinde, otellerde, hastanelerde ve kamu binalarında günlük hayatın ayrılmaz bir parçasıdır. Dikey ulaşımın bel kemiğini oluşturan bu sistemler, yalnızca bir taşıma aracı değil, aynı zamanda kullanıcı güvenliğini, konforunu ve bina içi trafiğin düzenini belirleyen kritik bir bileşendir. Withmor insan asansörleri, EN-81 standartlarına ve yerel yönetmeliklere tam uyumlu, yüksek güvenlikli ve enerji verimli çözümler olarak tasarlanır.

Her bina, kullanım amacı, kat sayısı, kullanıcı yoğunluğu ve mimari yapısı itibarıyla farklıdır. Bu nedenle yolcu asansörü projelerinde, tek tip bir çözüm yerine, binaya özel mühendislik yaklaşımı gerekiyor. Withmor mühendisleri, proje başlangıcında trafik analizleri yaparak yoğunluk saatlerini, hedef bekleme sürelerini, kabin kapasitesini ve asansör sayısını belirler. Böylece sabah–akşam yoğunluğu olan ofis binalarında, gün boyu dengeli trafikli konut bloklarında veya 7/24 aktif otel projelerinde, her biri için doğru kapasite ve hız kombinasyonu oluşturulur.

Kabin içi tasarım, kullanıcı deneyimini doğrudan etkileyen bir unsurdur. Paslanmaz çelik, cam, ahşap ve özel dekoratif panellerden oluşan kabin seçenekleri, projenin mimari diline uygun şekilde seçilir. Aydınlatma senaryoları, zemin kaplamaları, ayna düzeni ve buton dizaynı ile hem estetik hem de fonksiyonel bir iç mekân kurgulanır. Özellikle otel ve rezidans projelerinde, insanların asansörü sadece bir taşıma aracı değil, atmosferin bir parçası olarak algılaması için, kabin tasarımı titizlikle ele alınır.

Enerji verimliliği, modern insan asansörü sistemlerinde büyük önem taşır. Dişlisiz tahrik motorları, geri kazanımlı sürücüler ve akıllı kumanda algoritmaları sayesinde hem enerji tüketimi azaltılır hem de işletme maliyetleri kontrol altında tutulur. Grup kumanda sistemleriyle birden fazla asansörün birlikte çalıştığı projelerde, çağrıların en uygun kabine yönlendirilmesiyle gereksiz hareketler önlenir, bekleme süreleri kısaltılır ve asansör trafiği optimize edilir.

Güvenlik tarafında ise kapı kilitleme mekanizmaları, aşırı hız regülatörleri, tampon sistemleri, fotosel perdeler, acil durum haberleşme hatları ve kurtarma prosedürleri devrededir. Withmor insan asansörleri, bu bileşenlerin tamamını güncel standartlara göre sağlar ve devreye almadan önce kapsamlı testlerden geçirir. Ayrıca, periyodik bakım hizmetleri ile asansörlerinizin her zaman güvenli ve mevzuata uygun kalmasını sağlarız.

Withmor olarak, konut ve ticari projeleriniz için tasarladığımız insan asansörü sistemlerini, yük asansörleri, villa asansörleri ve yatay asansörler gibi diğer dikey ulaşım çözümleriyle uyumlu şekilde kurgularız. Böylece binanızın tüm dikey taşıma ihtiyaçlarını tek elden, bütüncül bir mühendislik yaklaşımıyla yönetebilirsiniz.`
    },
    {
      id: "konveyor",
      ...
      longDesc: `Konveyör asansörler, lojistik merkezleri, e-ticaret depoları, üretim tesisleri ve dağıtım merkezlerinde malzeme akışını otomatik hale getiren ileri seviye çözümlerdir. Koli, kasa, konteyner, palet ve benzeri birim yüklerin katlar arasında insan müdahalesine minimum ihtiyaçla taşınmasını sağlar. Withmor konveyör asansör sistemleri, üretim ve depo süreçlerinizdeki dikey taşıma ihtiyacını, konveyör hatlarıyla entegre çalışan akıllı bir sistem haline getirir.

Bu tip sistemlerde, sadece bir asansörden değil, tam anlamıyla bir “dikey lojistik hattı”ndan söz edilir. Konveyör asansör, giriş ve çıkış noktalarındaki bantlar, yönlendiriciler, sensörler ve kumanda panolarıyla birlikte düşünülmelidir. Malzeme, bant üzerinden belirlenen noktaya geldiğinde otomatik olarak kabine alınır, ilgili kata taşınır ve o katın konveyör hattına aktarılır. Böylece depo içi lojistik, merdiven veya klasik yük asansörleri ile yapılan manuel taşıma yerine, sürekli ve kesintisiz bir akışa dönüşür.

Withmor mühendislik ekibi, konveyör asansör projelerinde öncelikle tesisin mevcut veya planlanan iç lojistik şemasını inceler. Sipariş hazırlama alanları, raf yerleşimleri, paketleme istasyonları, giriş–çıkış rampaları ve sevkiyat noktaları detaylı şekilde analiz edilir. Bu analiz sonucunda konveyör asansörün taşıma kapasitesi, hız değeri, kabin ölçüleri ve giriş–çıkış yönleri belirlenir. Gerekiyorsa, çok noktalı giriş–çıkış senaryoları kurgulanarak esnek kullanım imkânı sağlanır.

Konveyör asansörlerde güvenlik, hem operatörler hem de ürünler için ayrı ayrı önemlidir. Foto sensörler, acil durdurma butonları, kabin ve kapı kilitleme mekanizmaları, aşırı yük algılama ve hatalı paket tespiti gibi özellikler, sistemin bir parçası olarak tasarlanır. Ayrıca, kontrol yazılımı ile depo yönetim sistemi (WMS) veya üretim planlama sistemleriyle haberleşme sağlanabilir. Böylece her bir birim yükün hangi kata, hangi zaman diliminde taşındığı takip edilebilir.

Enerji verimliliği açısından bakıldığında, konveyör asansörler sürekli çalışma rejimine uygun tasarlanır. Kullanılan motor ve sürücü grupları, uzun süreli çalışmalarda ısınma ve yıpranmayı minimuma indirecek şekilde seçilir. Mekanik aksamda kullanılan malzemeler, ağır hizmet koşullarına dayanacak dayanıklılıkta olup, düzenli bakım ile uzun yıllar sorunsuz hizmet verir.

Withmor konveyör asansör çözümleri, genel dikey ulaşım portföyümüzün önemli bir parçasıdır. Yük asansörleri, yük platformları ve yatay asansörlerle birlikte kurulduğunda, tesisinizde uçtan uca bir iç lojistik ağı oluşturabiliriz. Böylece hem insan gücüne bağımlılığı azaltır, hem hata riskini düşürür, hem de operasyon hızınızı artırırsınız.`
    },
    {
      id: "panoramik",
      ...
      longDesc: `Panoramik asansörler, binaların mimari kimliğini güçlendiren, kullanıcıya sadece dikey ulaşım değil aynı zamanda görsel bir deneyim sunan prestij çözümleridir. AVM’ler, oteller, iş merkezleri, rezidanslar ve kamusal alanlarda, yapıların vitrin yüzü olarak öne çıkarlar. Withmor panoramik asansörleri, cam kabinler, geniş görüş açıları ve özel aydınlatma tasarımları ile hem iç mekân hem de dış cephe estetiğini destekler.

Panoramik asansör projelerinde, taşıyıcı konstrüksiyonun mimariyle uyumu büyük önem taşır. Çelik, cam ve paslanmaz çelik kombinasyonlarıyla hem dayanıklı hem de şeffaf yapılar kurgulanır. Kule şeklinde dış cepheye monte edilen sistemlerde, bina siluetini güçlendiren, akşam saatlerinde aydınlatma senaryolarıyla dikkat çeken bir tasarım dili oluşturulur. İç mekânda konumlandırılan panoramik asansörlerde ise atrium boşlukları, galeri alanları ve lobi bölümleri ile bütünleşik çözümler geliştirilir.

Kabin tasarımı, panoramik asansörlerin en karakteristik unsurudur. Tam cam, yarım cam veya belirli bölümlerde cam paneller kullanılarak farklı şeffaflık seviyeleri elde edilir. Zemin ve tavan tasarımı, aydınlatma çizgileri, buton panelleri ve korkuluk detayları ile kullanıcıya ferah, modern ve güvenli bir ortam sunulur. Özellikle otel ve AVM projelerinde, kullanıcıların asansörü bir seyir alanı olarak deneyimlemesi hedeflenir; bu da marka algısına doğrudan katkı sağlar.

Teknik açıdan bakıldığında, panoramik asansörler de insan asansörleri gibi EN-81 standartlarına uygun güvenlik donanımlarıyla tasarlanır. Tahrik sistemi, kabin süspansiyonu, kapı mekanizmaları, hız regülatörleri ve emniyet tertibatları, açık görüşlü tasarımın gerektirdiği ilave mühendislik hesapları dikkate alınarak seçilir. Cam yüzeylerde kullanılan malzemeler, güvenlik standartlarına uygun temperli veya lamine camlardır; gerekli durumlarda renkli veya kaplamalı cam alternatifleri de kullanılabilir.

Panoramik asansörlerin enerji verimliliği de günümüz projelerinde önemli bir kriterdir. Dişlisiz motorlar, geri kazanımlı sürücüler ve akıllı kumanda sistemleri sayesinde, yoğun kullanım senaryolarında dahi enerji tüketimi kontrollü seviyelerde tutulur. Grup kumanda sistemleriyle birlikte farklı panoramik kabinlerin aynı atrium içinde dengeli şekilde çalışması sağlanabilir.

Withmor olarak panoramik asansör çözümlerini, yük asansörleri, villa asansörleri ve yatay asansörler gibi diğer dikey ulaşım çözümlerimizle uyumlu şekilde tasarlıyoruz. Böylece proje sahipleri, hem prestij alanlarında hem de servis–lojistik alanlarında tek elden yönetilen, bütüncül bir asansör altyapısına sahip oluyor. Panoramik asansörleriniz, sadece bugünün değil, geleceğin mimari beklentilerini de karşılayacak kalitede hayata geçiriliyor.`
    },
    {
      id: "yatay-yamac",
      ...
      longDesc: `Yatay ve yamaç asansörleri, özellikle eğimli arazilerde yer alan konut projeleri, oteller, sahil tesisleri ve peyzaj alanlarında erişilebilirliği artırmak için geliştirilen özel raylı sistemlerdir. Merdivenlerle çıkılması güç veya yorucu olan mesafeleri, konforlu ve güvenli bir yolculuğa dönüştürür. Withmor yatay–yamaç asansör çözümleri, topoğrafyası zor alanlarda dahi kullanıcıların günlük hareketini kolaylaştıran, proje değerini yükselten sistemler sunar.

Bu tür sistemler, klasik dikey asansörlerden farklı olarak hem yatay hem de eğimli doğrultuda hareket edebilir. Ray hattı, arazinin doğal eğimine ve peyzaj tasarımına uygun şekilde projelendirilir. Kabin, bu raylar üzerinde kontrollü bir şekilde hareket ederek, kullanıcıları alt kotlardan üst kotlara veya tam tersi yönde güvenle taşır. Özellikle sahil şeritlerinde, teraslı yerleşimlerde ve yamaç otellerinde, plaj–oda–restoran gibi noktalar arasında erişimi kolaylaştırmak için ideal bir çözümdür.

Yatay yamaç asansörü projelerinde en önemli aşama, arazi analizi ve mühendislik hesaplarıdır. Zemin yapısı, eğim oranı, rüzgâr yükleri, olası kayma veya erozyon riskleri gibi parametreler detaylı şekilde incelenir. Taşıyıcı konstrüksiyon, betonarme veya çelik ankrajlarla zemine sabitlenir; uzun yıllar boyunca güvenli bir hat oluşturacak şekilde tasarlanır. Kabin ve ray sistemlerinde kullanılan malzemeler, dış mekân koşullarına, UV etkisine, yağmur, kar ve rüzgâra dayanıklı özellikte seçilir.

Kullanıcı deneyimi açısından, yatay–yamaç asansörleri çoğu zaman açık manzaraya sahip güzergâhlarda çalıştığı için, kabin tasarımı da bu manzarayı en iyi şekilde sunacak şekilde kurgulanır. Cam korkuluklar, panoramik cam paneller ve ferah iç hacim ile yolculuk, sadece bir ulaşım değil, keyifli bir seyir deneyimine dönüşür. Otellerde misafirlerin odalarına, restoranlara veya spa alanlarına zahmetsizce ulaşmasını sağlayarak konforu artırır; konut projelerinde ise farklı kotlardaki yaşam alanları arasında güçlü bir bağ kurar.

Güvenlik tarafında ise hız kontrolü, fren sistemleri, acil durdurma mekanizmaları, kabin içi haberleşme ve hat boyunca konum izleme gibi unsurlar büyük önem taşır. Withmor yatay yamaç asansör sistemleri, ilgili güvenlik standartlarına uygun olarak tasarlanır ve devreye almadan önce kapsamlı testlerden geçirilir. Ayrıca düzenli bakım ve periyodik kontroller ile sistemlerin her mevsimde güvenli çalışması sağlanır.

Enerji verimliliği ve işletme maliyetleri de projelerde dikkate alınan kriterler arasındadır. Doğru seçilmiş motor ve sürücü grupları, eğimli hat boyunca kontrollü bir hız ve düşük enerji tüketimi sağlar. Bu sayede hem işletmeci hem de kullanıcı tarafında sürdürülebilir bir çözüm elde edilir. Yatay–yamaç asansörleri, villa asansörleri ve yük asansörleri gibi diğer dikey ulaşım çözümlerimizle birlikte ele alındığında, projenizde her kot için erişilebilir, güvenli ve konforlu bir ulaşım ağı oluşturmanıza yardımcı olur.`
    },
    {
      id: "ozel-projeler",
      name: "Özel Projeler",
      ...
      longDesc: `Özel projeler, standart katalog çözümlerinin sınırlarını aşan, binaya veya işletmeye özgü ihtiyaçların ön plana çıktığı uygulamaları ifade eder. Kuyu ölçülerinin alışılmışın dışında olduğu, mimari tasarımın sıra dışı çözümler gerektirdiği, taşıma senaryolarının klasik insan asansörleri veya yük asansörleri ile tam karşılanamadığı durumlarda, Withmor’un terzi işi mühendislik yaklaşımı devreye girer. Bu kapsamda, villa asansörleri, yatay asansörler, yük platformları ve panoramik çözümler gibi farklı teknolojileri harmanlayarak proje bazlı sistemler geliştiririz.

Özel proje çalışmalarında ilk adım, ihtiyacın doğru tanımlanmasıdır. Binanızın mimari projesi, statik raporları, kullanım senaryoları ve hedeflenen kullanıcı profili detaylı olarak incelenir. Bazı projelerde tarihi yapıların dokusuna zarar vermeden asansör entegrasyonu yapılması gerekirken, bazı projelerde ise dar kuyu ölçülerine maksimum kabin ölçüsünü sığdırmak öncelik haline gelir. Depo ve üretim tesislerinde, hem insan hem yük taşıma fonksiyonunun aynı hat üzerinde çözümlenmesi de sıkça karşılaşılan bir ihtiyaçtır.

Withmor mühendislik ekibi, bu tür projelerde klasik hazır çözümlerden ziyade, sıfırdan tasarım yaklaşımıyla çalışır. Konstrüksiyon yapısı, kabin formu, tahrik sistemi, kumanda senaryoları ve güvenlik donanımları proje özelinde belirlenir. Gerekirse hidrolik sistemler, makine dairesiz sistemler, yatay–yamaç çözümleri ve yük platformları bir arada kullanılarak hibrit sistemler oluşturulur. Böylece hem mimari hem de operasyonel gereksinimler tek bir bütün içinde karşılanır.

Özel projelerde kullanılan malzemeler ve imalat detayları, uzun ömürlülük ve güvenlik odağında seçilir. Proje alanına göre paslanmaz çelik, galvaniz kaplama, cam, kompozit paneller veya özel kaplama malzemeleri kullanılabilir. Kabin içi tasarım, projenin konseptine göre minimal, klasik veya modern çizgilerde kurgulanır. Bazı uygulamalarda tam cam kabinler ve özel aydınlatma senaryoları tercih edilirken, bazı projelerde endüstriyel kullanım için dayanıklılığı ön plana çıkaran tasarımlar öne çıkar.

Sürecin bir diğer önemli kısmı da resmi izinler ve mevzuat uyumudur. Withmor, özel projelerde ilgili belediye, yapı denetim firmaları ve yetkili kurumlarla koordineli bir çalışma yürütür. Statik hesaplar, ankraj detayları, asansör ruhsat süreçleri ve muayene prosedürleri proje takviminin bir parçası olarak planlanır. Böylece hem teknik hem idari açıdan sorunsuz ilerleyen bir süreç elde edilir.

Özel projeler kapsamında sunduğumuz çözümler, sadece bugünkü ihtiyacı karşılamakla kalmaz; binanın gelecekteki kullanım senaryolarını da göz önünde bulundurur. Gerekirse kapasite artışına izin verecek, modüler şekilde genişletilebilecek veya ileride farklı fonksiyonlarla birleştirilebilecek altyapılar kurgulanır. Yük asansörleri, villa asansörleri, panoramik sistemler ve yatay asansörler gibi farklı dikey ulaşım çözümlerini bir arada düşünerek, proje sahiplerine uzun vadeli, sürdürülebilir ve katma değeri yüksek bir mühendislik hizmeti sunarız.`
    },
  ]);
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
                <div className="text-xs text-slate-500">
                  Bu içerik SEO uyumlu olarak hazırlanmış olup, yönetici girişi ile panel üzerinden düzenlenebilir.
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => setActiveServiceModal(null)}
