...
export default function App() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const ensureWidgetRuns = () => {
        if (window.eapps?.platform?.run) {
          window.eapps.platform.run();
        }
      };

      const target = document.getElementById("social-feed");
      ...
      const timer = setTimeout(ensureWidgetRuns, 1200);
  ...
  const handleImageError = (e) => {
   ...
   return (
    ...
    <div className="min-h-screen bg-white text-slate-800 font-sans selection:bg-blue-100 overflow-x-hidden">
        ...
        <header className="sticky top-0 inset-x-0 z-50 w-full border-b border-slate-200 bg-white/95 backdrop-blur-xl shadow-[0_8px_30px_rgba(15,23,42,0.08)] supports-[backdrop-filter]:bg-white/80">
          ...
          <div>
            ...
            <div className="mb-6 flex flex-wrap gap-3">
              {heroBadges.map((badge) => (
                <div key={badge.text} className="relative inline-flex">
                  <span
                    className="absolute inset-[-4px] rounded-full bg-[conic-gradient(at_50%_50%,#ff3737,#ffb347,#2dd4bf,#3b82f6,#a855f7,#ff3737)] opacity-95 animate-border-flow blur-[3px]"
                  />
                  <span className="relative inline-flex items-center gap-2 rounded-full bg-white px-3.5 py-1.5 text-[11px] font-bold text-slate-800 border border-white/70 shadow-sm backdrop-blur-sm">
                    {badge.content}
                  </span>
                </div>
              ))}
              ...
              {/* SAĞ TARAF: Form ve Harita */}
                <div className="flex flex-col gap-8">
                  ...
                  <div className="relative group z-10">
                      <div className="absolute -inset-[7px] rounded-2xl bg-gradient-to-r from-red-600 via-blue-600 to-green-600 opacity-90 blur-lg transition duration-1000 group-hover:opacity-100 group-hover:duration-300 animate-tilt"></div>
                      <div className="relative overflow-hidden rounded-2xl p-[6px]">
                        <span className="absolute inset-[-105%] animate-[spin_3s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#FF0000_0%,#FFFF00_14%,#00FF00_28%,#00FFFF_42%,#0000FF_57%,#FF00FF_71%,#FF0000_85%,#FF0000_100%)]" />
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="text-center mb-8">
            ...
            <div className="relative group">
              <div className="absolute -inset-[5px] rounded-2xl bg-[conic-gradient(at_50%_50%,#ff3737,#ffb347,#2dd4bf,#3b82f6,#a855f7,#ff3737)] opacity-70 blur-sm group-hover:opacity-90 animate-border-flow" />
